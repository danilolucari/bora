...
- **Ambiente (atualizado 2026-08-20)**: o CLI **`firebase` 15.28.1 já está instalado** e no PATH (`~/.local/bin/firebase`; o prefix do npm foi apontado para `~/.local`, que o `.bashrc` já exporta, evitando sudo). **Emulador de Auth verificado no ar** nesta máquina: `firebase emulators:start --only auth --project demo-bora` responde em `localhost:9099` com `ready: true`, cruzando com as portas de `firebase.json`. **JDK 21 instalado** (Temurin 21.0.12.1 LTS em `~/.local/lib/jvm/current`, symlink versionado; `JAVA_HOME` e PATH exportados em `~/.bashrc` **e** `~/.profile`, porque o `.bashrc` tem early return em shell não-interativo). `firebase-tools` 15.28.1 exige Java 21+ — um JDK 17 é recusado. **Emulator Suite completo verificado no ar**: Firestore responde em `localhost:8080` e Auth em `localhost:9099`, com `All emulators ready!`.
- **⚠️ Achado que reabre a checklist manual (2026-08-20)**: esta máquina **tem** os meios que o plano supunha ausentes — `flutter devices` lista um **Android físico conectado** (moto g15, Android 15 / API 35) **e Chrome 151** (`/usr/bin/google-chrome`). Ou seja, as cinco verificações M — incluindo **R-1**, a única capaz de derrubar o AD-004 — são executáveis agora, não dependem mais de ambiente. `CHROME_EXECUTABLE` está vazio, mas o Flutter detecta o Chrome sozinho.
